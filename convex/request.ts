...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        .withIndex("by_receiver_sender", q => q.eq(
        ...
        const friends1 = await ctx.db.query("friends").withIndex("by_user1", (q) =>  
            q.eq("user1", currentUser._id)
        ).collect();

        const friends2 = await ctx.db.query("friends").withIndex("by_user2", (q) =>  
            q.eq("user2", currentUser._id)
        ).collect();

        if(friends1.some(friend => friend.user2 === receiver._id) || friends2.some((frined) => frined.user1 === receiver._id)){
            throw new ConvexError("You are already friends with this usesr");
        }
